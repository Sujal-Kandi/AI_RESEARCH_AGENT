...
def _missing_angles(queries: List[str]) -> Tuple[bool , bool]:
    ...
    so this doesn't trust the prompt alone - it verifies , and if either angle is missing,
    strategist_node appends a deterministic fallback query below rather than 
    re-asking the LLM. Bare "vs" is deliberately NOT used as the comparative signal - "ODI debut date vs which opponent"
    matched it during testing and isn't a comparison at all. Requiring the word "compar" itself
    trades a few missed legimate comparisons for zero false confirmations-the safe side ,
    since a missed one just costs one harmless extra query, while a false confirmation would let the guarantee silently do nothing.
    """
